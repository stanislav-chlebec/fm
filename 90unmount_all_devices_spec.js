describe('Unmount all mounted devices', function() { 
  ...
  it('unmounts all devices', function() { 
    ...
    cy.get('@getAllStatusCli').then((xhr1) => {
      var cliDev = xhr1.response.body.topology[0].node
      cy.get('@getAllStatusNetconf').then((xhr2) => {
        const netconfDev = xhr2.response.body.topology[0].node
        cy.get('table tbody tr td:first-child').should('have.length', cliDev.length + netconfDev.length)
      })
    })
